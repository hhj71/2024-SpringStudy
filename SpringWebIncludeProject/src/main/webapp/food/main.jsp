     ...
     <div class="text-center">
       <ul class="pagination">
        <c:if test="${startPage>1 }">
         <li><a href="../food/main.do?page=${startPage-1 }">&lt;</a></li>
        </c:if>
        
        <c:forEach var="i" begin="${startPage }" end="${endPage }">
          <li ${i==curpage?"class=active":"" }><a href="../food/main.do?page=${i }">${i }</a></li>
        </c:forEach>
        
        <c:if test="${endPage<totalpage }">
         <li><a href="../food/main.do?page=${endPage + 1}">&gt;</a></li>
        </c:if>
       </ul>
     </div>
   </div>
      ...
      <c:forEach var="vo" items="${cList }" varStatus="s">
         <c:if test="${s.index<9 }">
          <a href="../food/detail.do?fno=${vo.fno }">
           <img src="https://www.menupan.com${vo.poster}" style="width: 100px;height: 100px"
             title="${vo.name }">
          </a>
         </c:if>
      </c:forEach>
